...
const DownloadStoryScreen = ({ navigation }) => {
  //state
  let allStories = useSelector((_) => _.library.download);
  ...
  const renderStory = ({ item, index, separators }) => {
    const { name, slug, cover, chapters = [], lastChapter = {} } = item || {};
    ...
    return (
      <StoryListItem
        ...
        title={name}
        subTitle={[
          `Chương ${lastChapter.chapterId}: ${lastChapter.name}`,
          `Ngày cập nhật: ${moment(lastChapter.createdAt).format('YYYY-MM-DD HH:MM:SS')}`,
        ]}
